...
const PortfolioTable: React.FC<PortfolioTableProps> = ({ selectedDate, fundId, isLive }) => {
  const { auth } = useAuth();
  ...
  const saveTableConfig = async () => {
    ...
    try {
      const token = await auth.getToken();
      await axios.post('/api/table-config/portfolio', {
        fundId,
        config: localConfig
      }, {
        headers: { Authorization: `Bearer ${token}` }
  ...
  const loadPositions = async () => {
    ...
    try {
      ...
      const fxPositions: PortfolioPosition[] = fxResponse.data.data.map((pos: any) => ({
        ...pos,
        trade_type: 'FX Forward' as const
      }));

      const fxOptionPositions: PortfolioPosition[] = fxOptionsResponse.data.data.map((pos: any) => ({
        ...pos,
      ...
      setError(err.response?.data?.detail?.error || err.message || 'Failed to load positions');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return <div className="flex justify-center p-8">Loading positions...</div>;
  }
    ...
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-4 p-4 bg-gray-100 dark:bg-gray-800 rounded">
        <div className="flex items-center gap-4">
          <h3 className="text-lg font-semibold">Portfolio Positions ({positions.length})</h3>
          {!isEditing ? (
            <button
        ...
        <table className="w-full border-collapse border border-gray-300 dark:border-gray-600">
              ...
              {visibleColumns.map(col => (
                ...
                >
                  <div className="flex items-center gap-1">
                    {col.label}
                    {localConfig?.sorting.column === col.key && (
                      <span>{localConfig.sorting.direction === 'asc' ? '↑' : '↓'}</span>
                    )}
                  </div>
                </th>
              ))}
          ...
          <tbody>
            {sortedPositions.map((pos, index) => (
              <tr key={`${pos.trade_type}-${pos.trade_id}`} className={index % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800'}>
                {visibleColumns.map(col => (
                  <td
                    key={col.key}
                    className="border border-gray-300 dark:border-gray-600 px-3 py-2"
                  >
                    {formatValue(pos[col.key as keyof PortfolioPosition], col.key)}
                  </td>
                ))}
              </tr>
            ))}
          ...
          <tfoot>
            <tr className="bg-blue-100 dark:bg-blue-900 font-semibold">
              <td colSpan={visibleColumns.length} className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-center">
                P&L Summary
              </td>
            </tr>
            <tr className="bg-blue-50 dark:bg-blue-800">
              {visibleColumns.map(col => {
                if (col.key.startsWith('pnl') || col.key.includes('_pnl')) {
                  const pnlKey = col.key as keyof typeof pnlSummary;
                  return (
                    <td key={col.key} className="border border-gray-300 dark:border-gray-600 px-3 py-2 font-semibold">
                      {formatValue(pnlSummary[pnlKey], col.key)}
                    </td>
                  );
                }
                return <td key={col.key} className="border border-gray-300 dark:border-gray-600 px-3 py-2"></td>;
              })}
            </tr>
          </tfoot>
        </table>
