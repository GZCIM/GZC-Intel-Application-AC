...
export const ComponentPortalModal: React.FC<ComponentPortalModalProps> = ({
    ...
    const [port3200Components, setPort3200Components] = useState<
        Port3200Component[]
    >([]);
    const [localComponents, setLocalComponents] = useState<any[]>([]);
    const [loading, setLoading] = useState(false);
    const [importStatus, setImportStatus] = useState<Map<string, string>>(
        new Map()
    );
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (componentInventory.getAllComponents().length === 0) {
                console.error(
                    "ComponentInventory not initialized! This is a critical error."
                );
    ...
    const loadComponents = async () => {
        if (activeTab === "local") {
            ...
            if (componentInventory.getAllComponents().length === 0) {
                console.warn(
                    "ComponentPortalModal: Inventory empty, forcing rebuild..."
                );
            ...
            const filtered = (
                ...
                ["bloomberg-volatility", "portfolio"].includes(c.id)
            );
            console.log(
                "ComponentPortalModal: All components from inventory:",
                allComponents
            );
            console.log(
                "ComponentPortalModal: Search components result:",
                searchComponents
            );
            console.log(
                "ComponentPortalModal: Portfolio component:",
                componentInventory.getComponent("portfolio")
            );
            console.log(
                "ComponentPortalModal: Components length:",
            ...
            if (componentsToUse && componentsToUse.length > 0) {
                console.log(
                    "ComponentPortalModal: Using components:",
                    componentsToUse
                );
                setLocalComponents(componentsToUse);
            } else {
                console.error(
                    "ComponentPortalModal: No components found! Inventory initialization failed"
                );
        ...
        try {
            ...
            if (result.success) {
                setImportStatus((prev) =>
                    new Map(prev).set(componentId, "success")
                );
                ...
                setImportStatus((prev) =>
                    new Map(prev).set(componentId, "failed")
                );
            }
        } catch (error) {
    ...
    const handleLocalSelect = (componentId: string) => {
        console.log(
            "🎯 ComponentPortalModal: handleLocalSelect called with:",
            componentId
        );
        console.log(
            "🎯 ComponentPortalModal: Available components:",
            localComponents.map((c) => c.id)
        );
        ...
        try {
            ...
            console.error(
                "❌ ComponentPortalModal: Error in handleLocalSelect:",
                error
            );
        }
    };
