...
class EnhancedConfigService {
    ...
    private captureCurrentState(): Partial<EnhancedUserConfiguration> {
        ...
        const currentTheme =
            localStorage.getItem("selectedTheme") || "gzc-dark";
    ...
    private getCurrentTabLayoutData() {
        try {
            ...
            const tabLayouts = JSON.parse(
                localStorage.getItem("tabLayouts") || "[]"
            );
            const currentLayoutId =
                localStorage.getItem("currentLayoutId") || "default";
            const activeTabId =
                localStorage.getItem("activeTabId") || "analytics";

            const currentLayout = tabLayouts.find(
                (l: any) => l.id === currentLayoutId
            );
    ...
    private captureComponentStates(): ComponentState[] {
        ...
        try {
            ...
            const componentMemory = JSON.parse(
                localStorage.getItem("componentMemory") || "{}"
            );
            ...
            const bloombergState = sessionStorage.getItem(
                "bloomberg-volatility-state"
            );
            if (bloombergState) {
                states.push({
    ...
    async saveCompleteConfiguration(): Promise<void> {
        // Do not attempt cloud saves while editing is locked
        if (!editingLockService.isUnlocked()) {
            console.log(
                "⏭️ Skipping complete configuration save: editing is locked"
            );
            return;
        }
        const maxRetries = 3;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                // Prevent empty/unsafe payloads that would trigger backend 409s
                const hasTabs =
                ...
                const hasComponentStates =
                    Array.isArray(config.componentStates) &&
                    config.componentStates.length > 0;
                if (
                    !hasTabs &&
                    !hasLayouts &&
                    !hasPrefs &&
                    ...
                    console.log(
                        "⏭️ Skipping save: empty payload (tabs/layouts/prefs/states are empty)"
                    );
                    return;
                }
                console.log(
                    `💾 Saving complete configuration (attempt ${attempt}/${maxRetries}):`,
                    {
        ...
        // All retries failed
        console.error(
            "❌ Failed to save complete configuration after",
            maxRetries,
            "attempts:",
            lastError
        );
    ...
    private setupWindowStateTracking(): void {
        ...
        const debouncedSave = () => {
            if (!editingLockService.isUnlocked()) return;
        ...
        document.addEventListener("visibilitychange", () => {
            if (document.hidden) {
                ...
                if (editingLockService.isUnlocked()) {
                    ...
                    console.log(
                        "⏭️ Skipping visibility save: editing is locked"
                    );
                }
            }
        });
