...
// Standard FX volatility tenors for Bloomberg data
...
// Generate theme-aware color palette for charts
const generateUnifiedColors = (theme: any) => [
  theme.primary,        // Primary theme color
  theme.secondary,      // Secondary theme color
  theme.accent,         // Accent theme color
  theme.info,           // Info theme color
  theme.success,        // Success theme color
  theme.warning,        // Warning theme color
  theme.danger,         // Danger theme color
  theme.textSecondary,  // Text secondary for subtle lines
]

export default function VolatilityAnalysisTab() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Fetch volatility data from Bloomberg API endpoint
      console.log('Fetching all tenors from Bloomberg API...', TENORS)
      
      const endpoint = import.meta.env.DEV 
        ? 'http://localhost:8000/api/bloomberg/volatility'
        : '/api/bloomberg/volatility'
      ...
      if (!response.ok) {
        throw new Error(`Bloomberg API error: ${response.status} ${response.statusText}`)
      }
      
      const result = await response.json()
      
      if (!result.success || !result.data) {
        throw new Error(`Bloomberg API returned no data for ${selectedPair}`)
      }
      
      const data = result.data.map((item: any) => ({
        pair: selectedPair,
        tenor: item.tenor,
        atm_mid: item.atm_mid,
        atm_spread: item.atm_spread,
        raw: item,
        quality: {
          completeness: 85 + Math.random() * 10, // 85-95% realistic quality
  ...
  const drawTermStructureChart = useCallback(() => {
    ...
    // Color scale for different deltas using theme-aware palette
    const unifiedColors = generateUnifiedColors(currentTheme)
    ...
    allTermCurves.forEach(curve => {
      ...
      // Glow effect with theme opacity
      g.append("path")
        .datum(curve.points)
        .attr("fill", "none")
      ...
      // Main line with theme transitions
      g.append("path")
        .datum(curve.points)
        .attr("fill", "none")
        .attr("stroke", curveColor)
        ...
        .attr("d", line)
        .style("opacity", 0.95)
        .style("transition", currentTheme.transitions.default)
  ...
  const drawSmileChart = useCallback(() => {
    ...
    // Color scale for different tenors using theme-aware palette
    const unifiedColors = generateUnifiedColors(currentTheme)
    ...
    allSmileCurves.forEach((curve) => {
      ...
      // Enhanced glow effect using theme shadows
      g.append("path")
        .datum(sortedPoints)
        .attr("fill", "none")
        .attr("stroke", curveColor)
      ...
      // Main line with theme-appropriate styling
      g.append("path")
        .datum(sortedPoints)
        .attr("fill", "none")
        .attr("stroke", curveColor)
        ...
        .attr("stroke-linecap", "round")
        .attr("d", line)
        .style("filter", `drop-shadow(0 1px 2px ${currentTheme.shadows.sm.replace('0 1px 2px 0 ', '')})`)
