...
export interface ComponentMetadata {
  id: string
  displayName: string
  category?: 'core' | 'analytics' | 'trading' | 'external' | 'utility' | 'bloomberg'
  gridConfig?: {
...
export const enhancedComponentRegistry: Record<string, {
  ...
  // Bloomberg components - ID must match ComponentInventory
  'bloomberg-volatility': {
    ...
    metadata: {
      ...
      gridConfig: {
        defaultWidgets: [
          { viewId: 'surface3d', title: 'Volatility Surface 3D', icon: 'trending-up', defaultSize: { w: 6, h: 4 } },
          { viewId: 'surfaceTable', title: 'Volatility Grid', icon: 'grid', defaultSize: { w: 6, h: 4 } },
          { viewId: 'dataQuality', title: 'Data Quality', icon: 'activity', defaultSize: { w: 3, h: 2 } }
        ]
      }
    }
  },
