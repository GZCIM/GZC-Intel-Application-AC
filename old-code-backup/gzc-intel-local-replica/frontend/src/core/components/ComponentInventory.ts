...
export class ComponentInventory {
  ...
  private initializeDefaults() {
    ...
    this.addCategory({
      ...
      color: '#FF6B00',
      subcategories: [
        { id: 'volatility', name: 'Volatility Analytics', description: 'FX options volatility surfaces and analysis', tags: ['volatility', 'options', 'fx', 'surface'] },
        { id: 'rates', name: 'Rate Analytics', description: 'Interest rate curves and analytics', tags: ['rates', 'curves', 'ois', 'swaps'] }
      ]
    })
...
console.log('=== CHECKING BLOOMBERG COMPONENT ===');
console.log('Bloomberg exists?', !!componentInventory.getComponent('bloomberg-volatility'));
if (!componentInventory.getComponent('bloomberg-volatility')) {
  console.log('Adding Bloomberg Volatility component...');
  componentInventory.addComponent({
    id: 'bloomberg-volatility',
    name: 'BloombergVolatility',
    displayName: 'Bloomberg Volatility Surface',
  ...
  console.log('Bloomberg component added successfully!');
  console.log('Total components now:', componentInventory.searchComponents('').length);
}

// Final check
console.log('=== FINAL INVENTORY STATUS ===');
console.log('All components:', componentInventory.searchComponents('').map(c => c.id));
